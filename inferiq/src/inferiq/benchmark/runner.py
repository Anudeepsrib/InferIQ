...
from inferiq.benchmark.metrics import BenchmarkMetrics, MetricsComputer, format_metrics_table
from inferiq.benchmark.profiler import CUDAProfiler
from inferiq.benchmark.workloads import WorkloadGenerator
from inferiq.config.settings import BenchmarkConfig, ModelConfig, Settings, get_settings
from inferiq.gateway.schemas import GenerateParams, GenerateResult, GPUStats, ModelBackend
from inferiq.utils.gpu import GPUMonitor
...
async def run_benchmark(
    ...
    if dry_run:
        # Lightweight fixture benchmark - no GPU, no real models, deterministic output
        logger.info("DRY RUN mode - generating mock benchmark results")
        import json
        from datetime import datetime
        from pathlib import Path
        ...
        # Use default or filtered models, or a placeholder
        demo_models = model_configs or settings.models[:1] or [type('M', (), {'name': 'demo-model', 'backend': 'mock'})()]

        for mc in demo_models[:3]:  # limit
            for plen in settings.benchmark.prompt_lengths[:2]:
                for bs in settings.benchmark.batch_sizes[:2]:
                    result = {
                        "model": getattr(mc, 'name', 'demo-model'),
                        "backend": getattr(getattr(mc, 'backend', 'mock'), 'value', 'mock'),
                        "prompt_length": plen,
                        "batch_size": bs,
                        "timestamp": timestamp,
                        "metrics": {
                            ...
                            "tokens_per_second": 124.7,
                            "cost_per_token_usd": 0.0000023,
                            "success_rate": 1.0,
                            "total_requests": settings.benchmark.measured_runs,
                        },
                        "dry_run": True,
                    }
                    fake_results.append(result)
                    fname = f"DRYRUN_{result['model']}_{result['backend']}_{plen}_{bs}_{timestamp}.json"
                    with open(output_dir / fname, "w") as f:
                        json.dump(result, f, indent=2)

        logger.info(f"Dry-run complete: wrote {len(fake_results)} mock result files to {output_dir}")
        return fake_results
